...
export const generateImageWithGemini = async (heroName: string, secretWord: string, storyText: string): Promise<string> => {
  console.log('Attempting to generate image with Imagen 3.0...');
  ...
  // Créer un prompt simple et efficace pour Imagen
  const prompt = `A beautiful anime-style illustration of ${heroName}, a ${heroAge}-year-old character, in a modern setting related to "${secretWord}". The image should be colorful, engaging, and suitable for all ages. High quality digital art with vibrant colors and detailed character design. Portrait orientation, no text or words in the image.`;
  ...
  try {
    ...
    console.log('Imagen 3.0 API response status:', response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error(`Imagen 3.0 API error: ${response.status} - ${errorText}`);
      clearTimeout(timeoutId);
      return getFallbackImage();
    }

    const data: ImagenResponse = await response.json();
    console.log('Imagen 3.0 API response received successfully');
    
    if (!data.candidates || data.candidates.length === 0) {
      console.warn('No image generated by Imagen 3.0 API, using fallback');
      clearTimeout(timeoutId);
      return getFallbackImage();
    }
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      console.warn('Imagen 3.0 API request timed out');
    }
