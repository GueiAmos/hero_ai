...
export const generateImageWithGemini = async (heroName: string, secretWord: string, storyText: string): Promise<string> => {
  ...
  // Return fallback image immediately if API key is not configured
  if (!GEMINI_API_KEY || GEMINI_API_KEY === 'your_api_key_here') {
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        console.warn('Imagen API request timed out, using fallback image');
      } else if (error.message.includes('Failed to fetch')) {
        console.warn('Network error connecting to Imagen API, using fallback image');
      } else if (error.message.includes('CORS')) {
        console.warn('CORS error with Imagen API, using fallback image');
      } else if (error.message.includes('NetworkError')) {
        console.warn('Network connectivity issue with Imagen API, using fallback image');
      }
    }
    
    // Always return a fallback image instead of throwing
    return getFallbackAnimeImage();
  }
};
